...
This is the complete list of namespaces & public targets available from `compose.mk`, along with their documentation.  Most documentation is pulled automatically from [the latest source](https://github.com/elo-enterprises/k8s-tools/tree/master/compose.mk).  Some important notes about how these targets work:
...
| --------------------- | --------------------------------------------------------------------------------------------------- |
| [*`io.*`* targets:](/k8s-tools/api#api-io)           | Misc text-formatters, timers, and other utilities                                                          |
| [*`docker.*`* targets:](/k8s-tools/api#api-docker)   | Simple helpers for working with docker.                                                                    |
| [*`flux.*`*](/k8s-tools/api#api-flux)                | Miniature workflow library / pipe wizard.                                                                  |
| [*`tux.*`*  targets:](/k8s-tools/api#api-tux)        | Embedded TUI support.                                                                                      |
| [*`mk.*`*  targets:](/k8s-tools/api#api-mk)       | Low-level helpers for working with make.                                                                   |
| [*`stream.*`* targets:](/k8s-tools/api#api-stream)   | Support for IO streams, including basic stuff with JSON, newline-delimited, and space-delimited formats.   |
...
See the [TUI documentation](/k8s-tools/compose.mk#embedded-tui) for a high-level overview of what this is and how it works.  For more details, [see the main TUI docs](/k8s-tools/compose.mk#embedded-tui), read on into the API, or see the [tux container spec in compose.mk](
...
See especially the [Platform Setup Example](/k8s-tools/demos#demo-platform-setup) for a more complete walk-through of motivation, starting from an example use-case.
...
The [public API for `tux.*` targets](/k8s-tools/api#api-tux) describes several lower-level operations that can safely be performed inside or outside of the TUI (i.e. on the docker host).
...
See the [API docs](/k8s-tools/api#tui-private-api)
...
This is the complete list of namespaces & public targets available from `k8s.mk`, along with their documentation.  Most documentation is pulled automatically from [the latest source](https://github.com/elo-enterprises/k8s-tools/tree/master/k8s.mk). First, some important notes about how these targets work:
